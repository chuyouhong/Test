...
# 也可以使用关键字参数
conn = pymysql.connect(host='121.40.33.21', port=3306, user='root', passwd='mixiguoji123', db='new_test', charset='utf8')
...
#     conn.select_db(DB_NAME)
    ...
    # 插入单条数据
    sql = 'INSERT INTO items values("%d","%s")' %(1,"jack")

    # 不建议直接拼接sql，占位符方面可能会出问题，execute提供了直接传值
    value = [2,'John']
    cursor.execute('INSERT INTO items values(%s,%s)',value)

    # 批量插入数据
    values = []
    for i in range(3, 20):
        values.append((i,'kkhff'+str(i)))
    cursor.executemany('INSERT INTO items values(%s,%s)',values)
